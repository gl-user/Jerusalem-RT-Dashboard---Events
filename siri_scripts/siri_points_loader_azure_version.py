...
from common.utils import (logger_setup, delete_old_log_files, DatabaseConnection,
                          exit_script, config_reader, map_log_level)
import sys
...
# SIRI_API_URL = "https://alerts.j.gl-urban.com/api/v1/misc/siri-transmitter?API-Key=34e72064-78e1-4602-93a8-78277248eee3"
# POLYGON_FILE= D:\Data Engineering Project\Tasks\first_task\src\updated_scripts\data\polygon2.pkl
...
def fetch_custom_siri_data(api_url: str) -> dict:
    try:
        global LAST_API_RESPONSE
        logger.debug("Fetching SIRI Data..")
        header = {"Accept-Encoding": "gzip"}
        ...
        if response.status_code==200:
            ...
            siri_data = response_json.get('data')
            data_size = response_json.get('data_size')
            logger.info(f"Fetched SIRI data,{data_size}")
            return siri_data
        else:
            response_json = response.json()
            message = response_json.get('message')
            logger.info(f"SIRI Data not fetched.'{message}'.")
...
if __name__ == "__main__":
    try:
        ...
        if database_connection.check_table_existance(table_to_check):
            ...
            while True:
                validated_data_actual = fetch_custom_siri_data(SIRI_API_URL)
                if validated_data_actual:
                    logger.info(f"Fetched and formatted_data SIRI Data..{len(validated_data_actual)}")
                ...
                exit_script(logger=logger, current_date=current_date, hour=3, minutes=30)
        else:
